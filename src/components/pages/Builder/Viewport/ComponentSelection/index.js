...
//		if(groupKey !== "Container") {
		if(groupKey !== "Container" && groupKey !== 'Header' && groupKey !== 'Footer') {

			// if groupkey exists, add to group object
			let groupKeyExists = categorizedTemplates.find(item => item.groupKey === groupKey);
			if(groupKeyExists) {
				let arrIndex = categorizedTemplates.findIndex(item => item.groupKey === groupKey);
				categorizedTemplates[arrIndex].templates.push({ variant, template: templateFunc })
			}
									...
									{group.templates.map(t => (
										<Grid
											...
											sx={{
												...
												boxShadow: '0 3px 14px 0 rgb(0 0 0 / 5%)',
												borderRadius: '4px',
												overflow: 'hidden',
											}}
											onClick={() => addComponent(t.template)}
										>
											{iconList[group.groupKey][t.variant]}
										</Grid>
									))}
								</Stack>
							</Stack>
