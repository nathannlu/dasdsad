...
export const Settings = () => {
    ...
    const onSwitchItem = (e, key, parentKey = '', componentIdx = -1) => {
        ...
        if (parentKey.length && componentIdx != -1) {
            // If inside array (Ex: features)
            setProp((props) => {
                props[parentKey].value[componentIdx][name].value =
                    !props[parentKey].value[componentIdx][name].value;
            });
        } else if (parentKey.length && componentIdx == -1) {
            // If value is an object (Ex. button)
            setProp((props) => {
                props[parentKey].value[name].value =
                    !props[parentKey].value[name].value;
    ...
    const onDeleteImage = async (key, parentKey = '', componentIdx = -1) => {
        let placeholderSrc = '';
        if (
            selected.name === 'Hero_A' ||
            selected.name === 'Hero_B' ||
            selected.name === 'Hero_C'
        )
            placeholderSrc = 'https://dummyimage.com/720x600';
        else if (selected.name === 'Header_A' || selected.name === 'Footer_A')
            placeholderSrc = 'https://via.placeholder.com/50';
        else if (selected.name === 'Content_B')
            placeholderSrc = 'https://dummyimage.com/720x400';
        else if (selected.name === 'Feature_C')
            placeholderSrc = 'https://dummyimage.com/460x500';
            ...
            <Box width="100%">
                {type === 'textfield' && (
                    <TextField
                        name={key}
                        onChange={(e) =>
                            onChangeItem(e, key, parentKey, componentIdx)
                        }
                ...
                {type === 'image' && (
                    <>
                        {imagePlaceHolders.some((img) =>
                            component[key].value.includes(img)
                        ) ? (
                            <Widget
                            ...
                            <Button
                                ...
                                size="small"
                                onClick={() =>
                                    onDeleteImage(key, parentKey, componentIdx)
                                }>
                                Delete
                            </Button>
                        )}
    ...
    return <Box>{generateSettings(props)}</Box>;
};
