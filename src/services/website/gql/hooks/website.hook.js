...
export const useUpdatePageData = ({ onCompleted, onError }) => {
    ...
    const [updatePageData, { data }] = useMutation(UPDATE_PAGE_DATA, {
        refetchQueries: [GET_WEBSITES, 'GetWebsites'],
...
export const useAddPage = ({ pageName, pageData, onCompleted, onError }) => {
    ...
    const [addPage, { data }] = useMutation(ADD_PAGE, {
        ...
        refetchQueries: [REAUTHENTICATE, 'Reauthenticate'],
        onCompleted: (data) => {
...
export const useDeletePage = ({ onCompleted, onError }) => {
    ...
    const [deletePage, { data }] = useMutation(DELETE_PAGE, {
        ...
        onCompleted: (data) => {
            ...
            setWebsite((prevState) => {
                const updatedPageList = prevState.pages.filter(
                    (page) => page.uid !== deletedPageId
                );
...
export const useSetWebsiteFavicon = ({
    websiteId,
    imageUrl,
    onCompleted,
    onError,
...
export const useUpdateWebsiteSEO = ({
    websiteId,
    data,
    onCompleted,
    onError,
...
export const useRemoveCustomDomain = ({ websiteId, domain, onError }) => {
    ...
    const [removeCustomDomain] = useMutation(REMOVE_CUSTOM_DOMAIN, {
        ...
        onCompleted: (data) => {
            let newWebsite = { ...website };
            let newDomains = [...newWebsite.domains];
            const indexOfDeleted = newDomains.findIndex(
                (x) => x.domain === domain
            );
...
export const useSetCustomDomain = ({
    ...
    const [setCustomDomain, { ...mutationResult }] = useMutation(
        SET_CUSTOM_DOMAIN,
        {
            variables: { websiteId, domain },
            onCompleted: (data) => {
                ...
                newWebsite.isCustomDomainActive =
                    !newWebsite.isCustomDomainActive;
                setWebsite(newWebsite);
            },
            onError,
...
export const useAddPageToPublish = ({ onError }) => {
    ...
    const [addPageToPublish, { ...mutationResult }] = useMutation(
        ADD_PAGE_TO_PUBLISH,
        {
            onCompleted: (data) => {
                let newWebsite = { ...website };
...
export const useRemovePageFromPublish = ({ onError }) => {
    ...
    const [removePageFromPublish, { ...mutationResult }] = useMutation(
        REMOVE_PAGE_FROM_PUBLISH,
        {
            onCompleted: (data) => {
                let newWebsite = { ...website };
                let newPublish = [...newWebsite.published];
                const indexOfDeleted = newPublish.findIndex(
                    (page) => page.name === data.removePageFromPublish.name
                );
...
export const useUpdateWebsiteCustom = ({ onError }) => {
    ...
    const [updateWebsiteCustom, { ...mutationResult }] = useMutation(
        UPDATE_WEBSITE_CUSTOM,
        {
            onCompleted: (data) => {
                let newWebsite = { ...website };
