...
import { CREATE_CONTRACT, GET_CONTRACTS, SET_BASE_URI, UPDATE_CONTRACT, SET_WHITELIST, GET_CONTRACT, DELETE_CONTRACT, SET_CACHE_HASH, SET_CANDY_ACCOUNT_ADDRESS } from '../contract.gql';
...
export const useSetCandyAccountAddress = ({ onCompleted, onError }) => {
	const { setContracts } = useContract();

	const [setCandyAccountAddress, { ...mutationResult }] = useMutation(SET_CANDY_ACCOUNT_ADDRESS, {
		onCompleted: async data => {
			const updated = data.setCandyAccountAddress
	...
	return [ setCandyAccountAddress, { ...mutationResult }]
};