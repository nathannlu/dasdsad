...
export const LayerManagerProvider = ({ children }) => {
    ...
    const addLayer = (newLayer) => {
        try {
            const indexOfNewLayer = layers.findIndex(
                (layer) => layer.name === newLayer.name
            );
            if (indexOfNewLayer !== -1)
                throw new Error('You cannot add layer with the same name');
        ...
        if (result.some((layer) => layer.name == 'Background')) {
            if (result.findIndex((layer) => layer.name == 'Background') !== 0) {
                addToast({
                    severity: 'warning',
                    message:
                        'Your Background layer is no longer the last layer. Any images covered by the Background layer will not appear in your generated collection.',
                });
            }
        }
        return result;
