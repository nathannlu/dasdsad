...
export const GeneratorProvider = ({ children }) => {
    ...
    const getLayerImageIndex = (layer) => {
        ...
        layer.images.forEach((image, idx) => {
            weights.push(
                parseInt(image.rarity.value) + (weights[idx - 1] || 0)
            );
        });
        ...
        return new Promise((resolve) => {
            try {
                ...
                ctx.drawImage(
                    ...
                    0,
                    0,
                    imageDimension.width > 1000 ? 1000 : imageDimension.width,
                    imageDimension.height > 1000 ? 1000 : imageDimension.height
                );
    ...
    const stackLayers = (ctx) => {
        return Promise.all(
            layers.map((layer) => {
                return drawOnCanvas(ctx, layer).then((res) => {
                    return res;
                });
            })
        );
    ...
    const validateForm = () => {
        try {
            if (size.value.length < 1)
                throw new Error('Collection Size value cannot be left empty');
            ...
            if (
                layers[0].images[0].image.naturalWidth <= 0 ||
                layers[0].images[0].image.naturalHeight <= 0
            ) {
            ...
            if (possibleCombination < size.value) {
                throw new Error(
                    `Possible combination is under the desired collection count (${possibleCombination}/${size.value})`
                );
            }
