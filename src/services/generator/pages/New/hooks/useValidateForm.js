...
export const useValidateForm = () => {
    ...
    const validateCollectionSize = () => {
        try {
            ...
            if (size.value < 10)
                throw new Error('Collection Size value cannot be less than 10');
    ...
    const validateLayers = () => {
        try {
            if (layers.length < 1)
                throw new Error(
                    'Layers cannot be left empty. Try adding "Background"'
                );
    ...
    const validateLayerTraits = () => {
        try {
            ...
            layers.forEach((layer) => {
                ...
                if (layer.images.includes((file) => file.type == 'video/mp4'))
                    videoCount++;
            });

            if (videoCount > 1)
                throw new Error(
                    `Video can exist in one layer only! You have videos in ${2} layers.`
                );
