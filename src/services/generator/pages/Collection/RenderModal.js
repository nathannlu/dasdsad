...
const RenderModal = ({ renderModalState, setRenderModalState }) => {
    const { canvasRef, isDownloading, autoSaveCount, renderIndex } =
        useGenerator();
    const {
        query: { layers },
    } = useLayerManager();
    ...
    return (
        <Dialog
            open={renderModalState}
            onClose={() => setRenderModalState(false)}>
            <DialogTitle variant="h4">
                Your collection is rendering... 🚀
            </DialogTitle>
            <DialogContent>
                <DialogContentText>
                ...
                <Stack padding="1em" marginTop="1em">
                    <canvas
                        ref={canvasRef}
                        width={
                            layers[0]?.images[0]?.image?.naturalWidth > 1000
                                ? 1000
                                : layers[0]?.images[0]?.image?.naturalWidth
                        }
                        height={
                            layers[0]?.images[0]?.image?.naturalHeight > 1000
                                ? 1000
                                : layers[0]?.images[0]?.image?.naturalHeight
                        }></canvas>
                </Stack>
            </DialogContent>
        </Dialog>
